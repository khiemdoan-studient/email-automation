...
### Gotchas
- **PDF delivery = the web app serves the bytes (v2.17.0)**: never route the PDF click through Drive's browser front-end. The saga: `setSharing` on the original throws (shared-with-me, can't reshare) -> public COPY still failed in `/view` (preview error) -> even `uc?export=download` failed for the recipient's browser while curl fetched the bytes fine. Root fix: `doGet` reads the file server-side (`_servePdfPage`) and returns a base64 -> Blob auto-download page. No sharing, no copies, no Drive UI. Oversize (>15MB) or unreadable -> falls back to the `uc?export=download` redirect. `_publishPublicPdfCopy`/`_ensureReportLinksFolder`/`setupReportLinksFolder` remain only because copies referenced by already-sent drafts live in the "Email Report Links" folder - do NOT delete that folder while such links are in flight.
