...
### Gotchas
- **GitHub Pages does NOT redirect across an owner/org transfer (v2.27.0)**: git URLs redirect, Pages does not. When `email-automation` moved from `khiemdoan-studient` to the `studient-data` org, `https://khiemdoan-studient.github.io/email-automation/r.html` became a hard 404 while the repo itself kept working, so every tracked email link and PDF button died silently. Diagnose with a plain `curl -o /dev/null -w "%{http_code}"` against both hosts. Fix: update `CONFIG.TRACKING_SHIM_URL` AND remember `_trackingShimUrl()` reads the Script Property FIRST; `_repointDeadShimHost_` now rewrites known-dead hosts at read time because a stale property is invisible and uneditable from outside the Apps Script editor. Links already sent are unrecoverable.
- **Two Drive trees with identical filenames defeat name-based search (v2.27.0)**: the weekly-report folder was COPIED into the Studient Reports shared drive rather than moved, so ~921 filenames exist in both the live root and the retired personal one, and IMs can see both. `DriveApp.getFilesByName` is corpus-scoped, not folder-scoped, so a lookup can silently attach last month's PDF from the dead tree. The v2.5.1 collision defense does not help (2+ hits only, and it disambiguates by school folder name, which is identical in both trees). `_isUnderConfiguredRoot_` now rejects any hit that does not chain up to `CONFIG.ROOT_FOLDER_ID`. Keep it tri-state: an unwalkable parent chain must KEEP the file, or a permissions gap blanks every teacher's report, which is the exact fragility v2.5.0 moved to search to avoid.
- **`ROOT_FOLDER_ID` and `ROOT_FOLDER_NAME` are a PAIR (v2.27.0)**: `getRootFolder()` tries the id, then falls back to `findFolderByName(ROOT_FOLDER_NAME)`. Updating the id alone means any transient id failure routes the whole app into whatever folder still answers to the old name, and it looks like it is working. A unit test fails if they drift.
- **The second writer is `timeback-data-pipeline/email_export_timeback.py` (identified v2.26.2)**: it writes All Teacher Metrics, Student Winners and Student Year Highlights in THIS spreadsheet using its own service account (`sa-timeback@...`, vs Studient's `service-account@...`), on its own schedule. Until its v0.30.0 it cleared those shared tabs and appended headerless rows with empty teacher names, destroying Studient's data every run. It is now non-destructive (`_merge_own_rows`) and skips writing while teacher names are blank (`_unusable_by_consumer`). If email data goes blank again, check that repo FIRST, then run `python email_only.py` in the Studient repo to repair. `scripts/check_email_data.py --strict` fails on the damage via `_probe_shared_tabs_not_clobbered`; run it before any email cycle.
- **ANOTHER producer can overwrite the email spreadsheet's tabs - check structure before blaming lookup code (v2.26.1)**: `All Teacher Metrics` and `Student Winners` were found with their header row gone and the teacher-name column blank, carrying `ScienceSIS` / `Alpha School Brownsville` rows (an Alpha/Timeback dataset) instead of the 9 public schools. This repo's writer emits a header and had not changed, so a sibling pipeline writes the same spreadsheet. Symptom is deceptive: `getTeacherMetricsForWeek` skips nameless rows, so everything reports "no metrics for this week" and admin emails mail zeros. Diagnose by reading row 1 + column B of the tab, not by re-reading Apps Script. Repair: run `email_only.py` in the pipeline repo (~1 min, rewrites all email tabs from BigQuery). `_metricsTabProblem_` now blocks runs and names that repair; do not downgrade it to a warning.
- **The roster is the MAP Master Roster, NOT the "Teacher Emails" tab (v2.26.0)**: that tab is a single A1 cell holding a stacked 9-way IMPORTRANGE and it fails two ways. (1) It loads partially at random - three reads minutes apart gave 9 campuses, then 2, then zero rows; a run landing on a partial load drafts for the wrong teacher set (this produced a `0.0/0.0/0/0` JHES admin email). (2) The source tabs do NOT share a column layout - Reading CCSD first/last/email = 25/26/28 (plus a `Teacher_Name` decoy at 27), AASP = 25/26/27, the rest 24/25/26 - so the stack mis-columns 2 of 10 campuses even when it loads. ALWAYS read `CONFIG.ROSTER_SOURCE_ID` directly and resolve columns BY HEADER per tab (`_rosterColIndex_`); never index-address these tabs. FIRST header match must win: AFES/AFMS carry a mostly-empty `Summer School Teacher Email` at col 30 beside the real `Teacher 1 Email` at 26, and a last-match resolver silently drops 4 of AFMS's 11 teachers. Blank/`#REF!` campus cells fall back to the tab-name map (`ROSTER_TAB_CAMPUS`). `_rosterHealthProblem_` refuses to run when a selected campus yields zero teachers - do not weaken it into a warning.
- **"Metrics data: Available" is NOT "your teachers have data" (v2.26.0)**: the metrics tab is per-teacher-week and different campuses land in different weeks (e.g. 2026-03-02 holds only Reading-district rows). Any availability message must be scoped to the teachers being drafted (`_weeksWithDataForTeachers_`), or a run silently skips everyone while reporting healthy.
- **NO google.script.run server callbacks from HTML dialogs (v2.25.2)**: with multiple Google accounts signed in, the dialog iframe binds google.script.run to the browser's DEFAULT account, not the account that opened the sheet -> "Authorization is required to perform that action" for anyone multi-signed-in (this org's normal state; same routing class as the v2.18.0 /exec problem). Interactive flows must use SYNCHRONOUS Ui prompts/alerts, which run inside the menu-click execution. HtmlService display-only output (no server callback) is fine.
- **Midweek + weekly PDFs share teacher folders AND date patterns (v2.25.0)**: `Midweek {Teacher} - {range}.pdf` sits next to `{Teacher} - {range}.pdf`. Any filename matching MUST discriminate: midweek templates carry `pdfPrefix: 'Midweek '` (exact-prefix required), weekly templates REJECT names starting with "Midweek ". The traversal fallback does both; never revert it to a bare contains-match.
- **Admin email filename is COUPLED to the pipeline's naming (v2.25.0 cross-repo drift surface)**: `_adminPdfFilename_` replicates parent `scripts/generate_pdf_reports.py` (`{ABBREV} Admin Report {M-D}.pdf`, M-D no leading zeros, flat in the "Studient Weekly Reports" Shared Drive, find-by-name). If the pipeline renames or re-folders those PDFs, update `_adminPdfFilename_` in the same cycle. Fail-soft covers a missing file, not a renamed pattern (every week would show "not found"). IMs need access to that Shared Drive for the link; campus stats render regardless.
- **Days-in-week is PER-RUN state (v2.25.0)**: `_daysInWeek` scales CONFIG.THRESHOLDS via `_scaledThresholds_`/`_metricCellColors_` and MUST be reset to 5 in the `finally` of every entry point that sets it (`_generateDraftsFlow_`, `confirmAdminEmails`). Never persist it; V8 warm isolates would otherwise leak a short week into the next run.
- **Sheets API `values().clear()` clears VALUES, NOT FORMATS (v2.24.1 date bug)**: rebuilding a tab over old content leaves stale cell formats behind - the v2.24.0 dashboard rendered date serials as `4623000%` because percent formats from the old static layout survived. Any tab rebuild via the API must reset formats (`updateCells` with `fields:"*"` + `unmergeCells`) or use Apps Script `sheet.clear()` (which does clear both), then set explicit numberFormats per region.
