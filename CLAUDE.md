...
- **Click tracking (v2.15.0-v2.17.0)** - the weekly PDF is a **tracked link, not an attachment**. Deployed as a Web App; the `/exec` URL is baked into `CONFIG.TRACKING_WEBAPP_URL` (Script Property `TRACKING_WEBAPP_URL` overrides). RE-VERSION the SAME deployment after any `Code.js` push (`clasp deploy -i <id>`), else the live `/exec` runs stale code. **PDF delivery (v2.17.0)**: `doGet` serves the PDF bytes ITSELF (`_servePdfPage`: server-side blob read as the web-app owner -> base64 -> auto-download page). Works on shared-with-me originals; NO sharing, NO public copies needed; Drive's flaky browser front-end is out of the click path entirely (three prior attempts that ended at a drive.google.com URL all failed for recipients). Fallbacks: oversize (>15MB)/unreadable file -> `uc?export=download` redirect; `getUrl()` failure at draft time -> attach original. The redirect page targets `window.top` (v2.15.1) so nothing loads framed.
...
- **Click tracking (v2.15.0-v2.16.0)**: web app DEPLOYED 2026-07-06, id `AKfycbzxwauuhinj9htVMrlgPBTDCQxSGaOgLPZO8a9mRNNKBx8d9R_SeDTMBl0bh6r2IBg`, `/exec` URL baked into `CONFIG.TRACKING_WEBAPP_URL` (Script Property `TRACKING_WEBAPP_URL` overrides). Script Properties: `TRACKING_HMAC_SECRET` (auto-gen; rotating it breaks already-sent links), `REPORT_LINKS_FOLDER_ID` (auto-created "Email Report Links" folder holding the public PDF copies). `appsscript.json` carries the `webapp` block. **After every `clasp push`, re-version the SAME deployment**: `clasp deploy -i AKfycbzx...SeDTMBl0bh6r2IBg -d "..."` - a bare `clasp deploy` mints a new `/exec` id and stales the baked URL. PDFs are linked (to owned public copies), not attached - any template copy still saying "Attached:" (Week 2 / Week 8 subjects) is now inaccurate; harmless, candidate copy fix.
