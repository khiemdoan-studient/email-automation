...
**v2.6.8**: HOTFIX — multi-token teacher name lookup in SC Final Email. John Bradley Apostol's email body rendered all "--" because his roster name has 3 tokens (`John Bradley Apostol`) but BQ + Year Teacher Totals tab use the 2-token name `John Apostol` (BQ drops middle names upstream). `generateScFinalEmailBody` did direct-map lookup `getYearTeacherTotals()[teacherNameLower]` → null. Fixed by routing both year-cumulative readers through the existing `lookupByName(obj, firstName, lastName, fullName)` helper at Code.js:967-992 (step 2 resolves `firstName.split(' ')[0] + ' ' + lastName` = "john apostol"). Same helper all 13 weekly templates use for `metricsArray` lookups; SC Final Email (v2.6.5) was the new feature that bypassed it. No NAME_ALIASES entry needed — fix is structural. 51 → 52 tests; new test asserts 3-token name resolves to 2-token map key.

**v2.6.7**: HOTFIX paired with parent v3.41.6. Multi-campus teacher row collision in `Year Teacher Totals` (Muntasir Hamid teaches JHMS + JRHS → 2 rows → Apps Script last-write-wins picked JRHS values 280 min / 37 lessons / 0 GL instead of correct 42,685 / 2,698 / 57). Parent now GROUP BYs by `teacher_name` only. This release adds matching sheet-side validator probe `_probe_no_duplicate_teacher_in_year_totals` and 3 Code.js polish items: replaced `&mdash;` with `:` (line 2909), inserted underlined "Teachers: please show your students this slide" sentence above SHOW THIS SLIDE button (lines 2891-2895), dropped subject mention from Spotlight 1 narrative (`renderNarrative` lines 2278-2290 — now: "mastered N grade levels this year" without "in [subject]"). Verified end-to-end: 0 duplicate teachers across 80; 36/36 parent reconcile PASS; multi-campus spotlight cross-campus selection working (Alicia Westcot rank 1 = David Parsons / Reading, rank 2 = Angel Raymundo Duarte / Vita).

**v2.6.6**: HOTFIX paired with parent v3.41.5. Added 2 BQ cross-validation probes to `scripts/check_email_data.py`: `_probe_year_totals_match_weekly_dashboard` (per-teacher Year Teacher Totals vs raw weekly_dashboard SUM, 1% tol) and `_probe_highlights_within_actuals` (each spotlight student's cumulative_lessons ≤ 1.05 × actual weekly_dashboard per-student SUM). Catches the v3.41.4 cross-product multiplication bug class at the email-automation validator layer. main() now creates a bigquery.Client (BQ scope added) and runs the 2 new probes after the v2.6.5 tab probes. --strict exits 1 on any failure. NO Code.js change — the buildBody + helpers were correct; the data they consumed (parent's Student Year Highlights tab) was wrong. After parent v3.41.5 + email_only.py, fresh email drafts show correct spotlight numbers (Tashanique rank 2: Kimberly 284 lessons → Richar Galvez-Sanchez 77 lessons; Carolina rank 1: 70 grade_levels → 2 grade_levels).

**v2.6.5**: NEW template `SC Final Email: Growth & Hardwork = Results` for the May year-end push (Motivention store closes May 8). Subject: `Motivention Store Closing Friday (+ Impressive Results)`. Year-cumulative summary per teacher: 3-metric Classroom Data Highlight Reel (Total Minutes / Total Lessons / Total Grade Levels Mastered, since 2025-09-01), 2 KPIs (Avg Mastered Lessons + Avg Mastered Grades per student), 2 student spotlight cards (rank 1 by year-cumulative grade_levels with subject; rank 2 by year-cumulative lessons), state-testing transition, Mindset Mini-Launches (Week 9-11 Canva links). **Cross-repo**: parent v3.41.4 ships 2 new BQ queries (`query_year_teacher_totals`, `query_student_year_highlights`) + 2 new writers + 2 new tabs (`Year Teacher Totals`, `Student Year Highlights`). The grade-levels-mastered SQL semantics reuse the same `placed_before_activity = TRUE` rule as `query_student_winners` to prevent drift. Code.js: 14th template + 2 new readers (with module-level cache + reset hooks) + 3 new HTML helpers (`buildYearHighlightReel`, `buildYearKpiStrip`, `buildStudentSpotlights`) + new `generateScFinalEmailBody`. Tests grew 45 → 51. `scripts/check_email_data.py` extended with 2 new tab probes. Parent's `email_only.py` populated the 2 tabs (80 teacher rows + 145 student rows) before deploy. Lesson: cross-repo features that need new data should use the `_xxxCache + reset hook` pattern in Apps Script to keep the buildBody signature unchanged (`(teacher, metricsArray, winnersArray)`) — the cache is the dependency-injection boundary.

**v2.6.4**: validateAllPdfs respects Config Date Range. After v2.6.3, user ran with Config set to 2026-04-20 and got TWO reports: week 04-20 (100% match) AND week 04-27 (0% match — upstream pipeline hadn't run yet for the upcoming week). v2.6.2 had hardcoded `getAvailableWeeks().slice(0, 2)`, structurally guaranteed to flag the future week as MISSING. Fix: validate ONLY the Config-selected week (`getConfigValue('Date Range')`) — that's what the bulk Generate run uses. Error if Config Date Range is not set. Renamed menu item: `Debug: Validate All PDFs (last 2 weeks)` → `Debug: Validate All PDFs (Config week)`. Now the validator answers the actual question: "if I run Generate right now, will any teacher fail because of a missing PDF?".

**v2.6.3**: validateAllPdfs traversal-fallback fix. After v2.6.2 deployed, user ran the validator and got 0/74 for current week + 50/74 for prior week, with many MISSING teachers (Bertha, Avlen, Verenice) whose PDFs are clearly visible in Drive. Root cause: validateAllPdfs only called `findTeacherPdfBySearch` but the bulk-Generate flow (`createDraftForTeacher`) does **search-first, traversal-fallback**. Drive's search API has known search-index lag for shared-with-me files — a PDF mark.katigbak just added isn't immediately findable via `getFilesByName` even though folder traversal works fine. Fix: mirror the EXACT two-phase lookup as `createDraftForTeacher`. Now reports MISSING only if BOTH search AND traversal return null — same as what the bulk run would actually fail on. Added `foundViaFallback` counter shown in per-week stats (e.g. "12 via traversal fallback — search-index lag"). Lesson: when mirroring an existing flow, mirror the WHOLE flow, not just the headline call.
